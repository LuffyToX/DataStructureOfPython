...
class Suffix:
    ...
    def splitExpressionToList(self):
        """ 将表达式切分成项列表（仅支持每个项以空格分隔的情形）"""

        return self.line.split()

    def transformInfixToSufix(self):
        """ 将中缀表达式转换为后缀表达式 """
        ...
        items = self.splitExpressionToList()                      # items 是中缀表达式的各项
        for x in items:
    ...
    def suffixExpressionCalculation(self):
        """ 计算后缀表达式 """

        suffixOperators = "+-*/"  # 运算符
        ss = ESStack()

        items = self.transformInfixToSufix()    # items 是转换后的后缀表达式的各项
        for x in items:
...
if __name__ == "__main__":
    expression = "( 5 + 3 ) * 6 / 2"
    suf = Suffix(expression)
    print(suf.suffixExpressionCalculation())
